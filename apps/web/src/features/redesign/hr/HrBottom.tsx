...
import { DEPTS, LOCATIONS, WORKFORCE_HEALTH } from "./data";

export function HrBottom() {
  return (
...
function DeptTableCard() {
  const [scope, setScope] = useState("All Depts");
  return (
    <div className="glow-card flex flex-col p-5">
      <CardHead
        title="Workforce by Department"
        selects={[{ value: scope, onChange: setScope, options: ["All Depts", "Hiring"] }]}
      />
        ...
        <table className="w-full min-w-[520px] text-left">
          <thead>
            <tr className="text-[11px] uppercase tracking-wide text-rd-muted">
              ...
              <th className="px-2 py-2 text-right font-medium">Attrition</th>
              ...
              <tr key={d.name} className="border-t border-rd-border/60">
                <td className="px-2 py-2.5">
                  <div className="flex items-center gap-2.5">
                    <AvatarStack names={[d.name]} size={30} />
                    <span className="truncate text-sm font-medium text-rd-heading">{d.name}</span>
                  </div>
                </td>
      ...
      <FooterLink label="View org breakdown" />
    </div>
  );
}
  ...
  return (
    <div className="glow-card flex flex-col p-5">
      <CardHead
        ...
        selects={[{ value: period, onChange: setPeriod, options: ["Current", "vs Last Qtr"] }]}
      />
      {/* ponytail: stylized map backdrop (glow dots on a faint grid), not a real
          geo map — the location list below carries the numbers. */}
      ...
      <ul className="flex-1 space-y-2.5">
        {LOCATIONS.map((l) => (
          <li key={l.label} className="flex items-center gap-2 text-sm">
            <span className="min-w-0 flex-1 truncate text-rd-body">{l.label}</span>
            <span className="shrink-0 tabular-nums text-rd-heading">{l.value}</span>
            <Delta value={l.delta} suffix="" className="w-12 justify-end" />
          </li>
        ))}
      </ul>
    </div>
...
function HealthCard() {
  const [scope, setScope] = useState("All Employees");
  const { total, slices, insight } = WORKFORCE_HEALTH;
  return (
    <div className="glow-card flex flex-col p-5">
      <CardHead
        title="Workforce Health"
        selects={[{ value: scope, onChange: setScope, options: ["All Employees", "At Risk"] }]}
      />
      ...
      <FooterLink label="View flight-risk list" />
    </div>
  );
}
